...
(2) Two or more principals may authorize the same agent to make separate contracts for them. If the agent makes a single contract with a third party on the principals' behalves that combines the principals' separate orders or interests and calls for a single performance by the third party, Might be relevant to cases where either one or more of the several Principals are AALE(s) mixed up with non-AALE in dealing with this Agent ; or, the Agent is an AALE badly engineered in ways that result in this contractual confusion or unauthorized conflagration.
