...
## Some remarks on "legal agent" status, "intent", and "control" in a context of Automated and Autonomous Systems (AAS)
...
& _**context-dependent**_ (the ecosystem it relates too within some specific implementation) ...
...
It goes without saying that PID-controllers-built Automated and Autonomous Systems (PID-AAS) 
is something that a subset of our engineer community has understood for more than half a century 
and knows how to do, from the Appolo 11 lunar lander to industrial automation to VR systems an expresso machine.
...
The following deals with some other aspects of complexity, granularity, and "guaranteed rabbit holes", 
...
while involving the right people in the right place (or bring them to the place where they needed to be).
...
"Could an Inventor-AI qualify for "Legal Agent" status?" once you've established the fact that 
...
Nonetheless, in that particular context, there are clearly existing legal & ultimately Computational Law implications.
...
and seem to remain only partially resolved from a B/L/T/ standpoint, in fact they're increasingly being challenged.

Furthermore, the issues of "intent" and "control" do come back in the picture with a vengeance. 
There is this privileged link between “intent” and NPI, since “intent” doesn’t exist legally for an AAS, only for an NPI) 
that is then deployed among the various bodies of Law. 
...
that might come back to haunt us in the design of AALEs. (Or maybe not).) 
...
However we don’t seem to exonerate ourselves from dealing with these questions in due time. 
